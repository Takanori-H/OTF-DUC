...
public class UpdatingControllersDefinition extends CompositionExpression {
	...
	protected CompositeState compose(Vector<Value> actuals) {
        ...
        if (this.getMapping() != null && !this.getMapping().toString().equals("unknown") && !this.getMapping().toString().isEmpty()) {
            String mappingName = this.getMapping().toString();
            mappingComponents = getComponentsWithoutComposition(mappingName);
            output.outln(" - Mapping components loaded from composite '" + mappingName + "': " + mappingComponents.size() + " machines.");
        }
		// ケースB: 新しいリスト形式指定 (oldEnv, newEnv, mapRel) がある場合
            ...
            // サイズ整合性チェック
            if (oldEnvironmentList.size() != newEnvironmentList.size() || oldEnvironmentList.size() != mapRelationList.size()) {
		...
		if(this.isOTF)
		{
            ...
            if(oldSafeCol != null) oldSafetyLTSs.addAll(oldSafeCol);

            // New Safety Goals の保持 (Vector変換)
        	Vector<CompactState> newSafetyLTSs = new Vector<>();
        	if(newSafeCol != null) newSafetyLTSs.addAll(newSafeCol);

			// ★変更: ここで Transition Goals を CompactState に変換する
            // 元の getTransitionGoals() (List<Symbol>) を渡して変換
            Vector<CompactState> transitionLTSs = UpdatingControllersUtils.compileTransitionRequirements(this.getTransitionGoals(), output);
            ...
            for (CompactState cs : mappingComponents) {
                if (cs.alphabet != null) {
                    for (String s : cs.alphabet) alphaSet.add(s);
                }
            }
            
            // (B) 更新プロセス固有のイベントを追加
            // これらをFluentが参照している場合(例: fluent UpdateMode = <hotswap_begin, hotswap_end>)
            // アルファベットに含まれていないと遷移が生成されないため、明示的に追加します。
            alphaSet.add(UpdateConstants.HOTSWAP_BEGIN); // "hotswap_begin"
            alphaSet.add(UpdateConstants.HOTSWAP_END);   // "hotswap_end"
            ...
            ucce = new UpdatingControllerCompositeState(oldC, newC, mappingComponents, newEnvComponents,
		...
		else
		{
            ...
            ControllerGoal<String> grGoal = UpdatingControllersUtils.generateGRUpdateGoal(this, oldGoalDef, newGoalDef, controllableSet);
            ControllerGoalDefinition safetyGoal = UpdatingControllersUtils.generateSafetyGoalDef(this, oldGoalDef, newGoalDef, controllableSet, output);
	...
	/**
     * Retrieves the components of a composite definition without performing parallel composition.
    ...
    private void ensureCompiled(String name, Hashtable<String, ProcessSpec> processes) {
        ...
        if (compiled.containsKey(name)) return;
    ...
    private void printLTSStructure(CompactState lts, LTSOutput output) {
        ...
        for (int i = 0; i < lts.maxStates; i++) {
            ...
            if (current == null) {
                ...
                while (current != null) {
                    if (!first) sb.append(", ");
                    String eventName = (current.event < lts.alphabet.length) ? lts.alphabet[current.event] : "unknown(" + current.event + ")";
     ...
     */
    private Map<List<Integer>, Integer> generateStateMapping(CompactState originalSafe, List<String> sortedErrorActions) {
            ...
            while (current != null) {
                ...
                while (nd != null) {
                    if (nd.next == Declaration.ERROR) {
                        ...
                        key.add(s);
                        for (String errorAction : sortedErrorActions) {
                            if (errorAction.equals(actionName)) key.add(1);
                            else key.add(0);
                        }
